/**
 * Fetches Culture Guides knowledge from Supabase and builds the system prompt.
 * Falls back to static culture-guides-knowledge.ts when Supabase is not configured or returns empty.
 */
import { getSupabaseClient, isSupabaseConfigured } from './supabase'
import {
  hubLeads as staticHubLeads,
  regionLeads as staticRegionLeads,
  getCultureGuidesSystemPrompt as getStaticPrompt,
} from './culture-guides-knowledge'

export async function getCultureGuidesSystemPrompt(today?: string): Promise<string> {
  if (!isSupabaseConfigured()) {
    return getStaticPrompt(today)
  }

  const supabase = getSupabaseClient()
  if (!supabase) return getStaticPrompt(today)

  try {
    const [hubRes, regionRes, sectionsRes] = await Promise.all([
      supabase.from('hub_leads').select('city, lead').order('city'),
      supabase.from('region_leads').select('region, lead').order('region'),
      supabase.from('knowledge_sections').select('slug, content').order('slug'),
    ])

    const hubRows = hubRes.data ?? []
    const regionRows = regionRes.data ?? []
    const sectionRows = sectionsRes.data ?? []

    // Use Supabase data only if we have rows; otherwise fall back to static
    if (hubRows.length > 0 || regionRows.length > 0 || sectionRows.length > 0) {
      const hubList =
        hubRows.length > 0
          ? hubRows.map((r) => `${r.city}: ${r.lead}`).join('\n')
          : Object.entries(staticHubLeads)
              .map(([hub, lead]) => `${hub}: ${lead}`)
              .join('\n')

      const regionList =
        regionRows.length > 0
          ? regionRows.map((r) => `${r.region}: ${r.lead}`).join('\n')
          : Object.entries(staticRegionLeads)
      ...
      const sectionsContent = sectionRows
        .filter((s) => s.content)
        .map((s) => s.content)
        .join('\n\n')

      return buildPromptFromData(todayStr, hubList, regionList, sectionsContent)
    }
  } catch (err) {
    console.warn('Supabase knowledge fetch failed, using static:', err)
  }

  return getStaticPrompt(today)
}

const STATIC_KEY_FACTS = `
KEY FACTS:
...
function buildPromptFromData(
  ...
  const basePrompt = `You are the Culture Guides Assistant for Salesforce. You are helpful, friendly, and knowledgeable.
...
`
  ...
  const keyFacts = sectionsContent ? `\n\nADDITIONAL KNOWLEDGE:\n${sectionsContent}` : STATIC_KEY_FACTS
  return basePrompt + keyFacts
}
